...
def pick_team(out, budget, position_column, points_column, name_column, price_column, formation=[4,4,2]):
    ...
    defenders = output_df.where(output_df[position_column] == 2).sort_values(points_column,ascending = False).head(formation[0])
    midfielders = output_df.where(output_df[position_column] == 3).sort_values(points_column,ascending = False).head(formation[1])
    strikers = output_df.where(output_df[position_column] == 4).sort_values(points_column,ascending = False).head(formation[2])
    ...
    while(team_value >= budget + 0.001):
        ...
        potential_replacements = pd.DataFrame(columns = ['init_name', 'init_price', 'init_points', 'new_name', 'new_price', 'new_points', 'pos'])

        # Find any players in the same position with a lower price, sorted descending
        for index, row in team.iterrows():
            pos = row[position_column]
            price = row[price_column]
            potential_replacement = output_df.where((output_df[position_column] == pos) & (output_df[price_column] < price)).sort_values(points_column,ascending = False).head(1)
        ...
        potential_replacements['points_hit'] = potential_replacements['init_points'] - potential_replacements['new_points']
        potential_replacements['price_boost'] = potential_replacements['init_price'] - potential_replacements['new_price']
        potential_replacements['points_price_ratio'] = potential_replacements['price_boost'] / potential_replacements['points_hit']
        ...
        print("Unfortunate " + replacement['init_name'] + " replaced with " + replacement['new_name'] + ", saving " + str(replacement['price_boost']) + " but losing " + str(replacement['points_hit']) + " points\n")
    ...
    while(team_value <= budget):
        ...
        for index, row in team.iterrows():
            ...
            potential_replacement = output_df.where((output_df[position_column] == pos) & ((output_df[price_column] - price) <= budget - team_value) & (output_df[points_column] > points)).sort_values(points_column,ascending = False).head(1)
        ...
        potential_replacements['points_boost'] = potential_replacements['new_points'] - potential_replacements['init_points']
        potential_replacements['price_hit'] = potential_replacements['new_price'] - potential_replacements['init_price']
        potential_replacements['points_price_ratio'] = potential_replacements['points_boost'] / potential_replacements['price_hit']
        
        #potential_replacements = potential_replacements.sort_values(by=['points_boost', 'price_hit'], ascending = False)
        potential_replacements = potential_replacements.sort_values(by=['points_boost', 'points_price_ratio'], ascending = False)
        ...
        print("Fortunate " + replacement['init_name'] + " replaced with " + replacement['new_name'] + ", costing " + str(replacement['price_hit']) + " but gaining " + str(replacement['points_boost']) + " points")
...
def pick_transfers(team, out, budget, position_column, points_column, name_column, price_column, formation=[4,4,2], max_transfers = 1000):
    ...
    while(team_value <= budget and transfers < max_transfers):
        ...
        print(replacement['init_name'] + " for " + replacement['new_name'] + " costing " + str(replacement['price_hit']) + " and gaining " + str(replacement['points_boost']) + " points")
...
def main(save_team, from_scratch, formation, budget, max_transfers):
    ...
    players = pd.merge(players,predictions,how='inner',left_on=['player', 'position'],right_on=['player', 'position'])
    if from_scratch:
        ...
        team = pick_transfers(team, players, budget, 'position', 'prediction', 'player', 'price', formation, max_transfers)
    if save_team:
        team.to_csv(source_path + 'selected_team.csv', index=False)   
    
if __name__ == "__main__":
    if len(sys.argv) != 6:
        print("Usage: python team_picker.py <save_team> <build_from_scratch> <formation> <budget> <max_transfers>. Eg: python teams_scraper.py 5000")
        sys.exit(1)
